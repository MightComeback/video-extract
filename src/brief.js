...
export function normalizeUrlLike(s) {
  ...
  function canonicalizeKnownProviderUrl(u) {
    ...
    if (host === 'loom.com' || host.endsWith('.loom.com') || host === 'useloom.com' || host.endsWith('.useloom.com')) {
  ...
  function normalizeBareKnownUrl(u) {
    ...
    const m = s.match(
      /^(?:www\.)?(?<host>(?:fathom\.video|loom\.com|useloom\.com|share\.loom\.com|player\.loom\.com|share\.useloom\.com|player\.useloom\.com|youtu\.be|(?:[a-z0-9-]+\.)*youtube\.com|(?:[a-z0-9-]+\.)*youtube-nocookie\.com|(?:player\.vimeo\.com|(?:[a-z0-9-]+\.)*vimeo\.com)))\/(?<rest>\S+)/i
    );
    if (!m) return '';
    ...
    if (normalizedHost === 'player.loom.com') normalizedHost = 'loom.com';

    // Provider parity: older Loom links sometimes use useloom.com.
    if (normalizedHost === 'useloom.com' || normalizedHost.endsWith('.useloom.com')) normalizedHost = 'loom.com';
    if (normalizedHost === 'share.useloom.com') normalizedHost = 'loom.com';
    if (normalizedHost === 'player.useloom.com') normalizedHost = 'loom.com';
  ...
  const md = v0.match(
    /^\[[^\]]*\]\(\s*(?<u>(?:(?:https?:\/\/|data:)[^)\s]+)|(?:(?:www\.)?(?:fathom\.video|loom\.com|useloom\.com|share\.loom\.com|player\.loom\.com|share\.useloom\.com|player\.useloom\.com|youtu\.be|(?:[a-z0-9-]+\.)*youtube\.com|(?:[a-z0-9-]+\.)*youtube-nocookie\.com|(?:player\.vimeo\.com|(?:[a-z0-9-]+\.)*vimeo\.com))\/[^)\s]+))\s*\)\s*[)\]>'\"`“”‘’»«›‹.,;:!?…。！，？。､、）】〉》」』}]*$/i
  );
  if (md) {
...
function extractEnvironment(transcript) {
  ...
  const browsers = ['chrome', 'firefox', 'safari', 'edge', 'brave', 'arc', 'opera', 'vivaldi', 'chromium', 'duckduckgo', 'samsung internet', 'orion', 'tor browser', 'zen', 'librewolf', 'ladybird', 'floorp', 'waterfox', 'opera gx', 'pale moon'];
  ...
  const environments = ['staging', 'production', 'prod', 'localhost', 'development', 'dev', 'qa', 'uat', 'preview', 'vercel', 'netlify'];
  ...
  for (const b of browsers) {
    if (new RegExp(`\\b${b}\\b`, 'i').test(s)) {
      ...
      else if (b === 'pale moon') hits.push('Pale Moon');
  ...
  for (const o of os) {
    if (new RegExp(`\\b${o.replace('.', '\\.')}\\b`, 'i').test(s)) {
      ...
      else if (o === 'psvr' || o === 'psvr2' || o === 'playstation vr') hits.push('PlayStation VR');
      ...
      else if (o === 'kali') hits.push('Kali Linux');
